...
export class RamModelService {
  ...
  getAllRamModels(): Observable<RamModelItem[]> {
    return this.http.get<RamModelItem[]>(this.apiUrl + 'getAllRamModels/').pipe(
      catchError(this.handleError)
    );
  }

  getRamModelById(id: number): Observable<RamModelItem> {
    return this.http.get<RamModelItem>(`${this.apiUrl + 'getRamModelById'}/${id}`).pipe(
      catchError(this.handleError)
    );
  }

  createRamModel(ramModel: CreateRamModelRequest): Observable<RamModelItem> {
    return this.http.post<RamModelItem>(this.apiUrl + 'createRamModel/', ramModel, { observe: 'body' }).pipe(
      catchError(this.handleError)
    );
  }

  updateRamModel(id: number, ramModel: UpdateRamModelRequest): Observable<RamModelItem> {
    return this.http.put<RamModelItem>(`${this.apiUrl + 'updateRamModel'}/${id}`, ramModel, { observe: 'body' }).pipe(
      catchError(this.handleError)
    );
  }

  deleteRamModel(id: number): Observable<any> {
    return this.http.delete(`${this.apiUrl + 'deleteRamModel'}/${id}`, { observe: 'response' }).pipe(
      catchError(this.handleError)
    );
  }
